...
(12 movement lanes), which match the V22 lane schema.  It is intentionally
...
def _load_net_metadata(network: Path):
    ...
    if len(green_indices) != 4:
        raise ValueError(
            f"V22 adapter requires exactly four protected green phases; "
    ...
    if len(inc_lanes) != 12:
        raise ValueError(
            f"V22 adapter requires exactly 12 incoming lanes; {network} has {len(inc_lanes)}"
        )
...
def _load_movement_mapping(
    ...
    """Map each route movement to a V22 logical lane using SUMO connections."""
...
def load_scenario(junction: str, env_name: str) -> Scenario:
    ...
    return Scenario(
        ...
        # phases.  We replace it with a V22-compatible four-group program
...
def _run_controller(
    ...
    logic = traci.trafficlight.Logic("cosydelay_v22_tso", 0, 0, phases)
    ...
    try:
        ...
        while True:
            ...
            # The full V22-compatible program already contains all durations;
...
def run(args: argparse.Namespace) -> int:
    ...
    model, symbolic_plan, webster_plan = _load_v22()
    plans = {}
    if "v22" in args.controllers:
        plans["v22"] = symbolic_plan(scenario.rates, model)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--controllers", nargs="+", choices=("v22", "webster"), default=["v22", "webster"])
