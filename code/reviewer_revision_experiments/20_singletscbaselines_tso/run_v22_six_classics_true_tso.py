"""True SUMO replay for CoSyDelay-V22 and six classical delay models.
...
GENERALIZATION_DIR = GMini / "reviewer_revision_experiments" / "17_v22_vs_classics_generalization"
...
def _load_specs() -> Dict[str, DelaySpec]:
    ...
    v22_path = SIGNAL_EXPERIMENT / "models" / "symbolic_lane_model.json"
    v22 = require_formal_matrix_model(v22_path)
    specs["v22"] = DelaySpec(
        name="CoSyDelay-V22",
        expression=v22.expression,
        lane_parameters=v22.lane_parameters,
    )
...
def _plan(spec_key: str, spec: DelaySpec, scenario: Scenario, v22_model: SymbolicModel) -> Dict[str, float]:
    if spec_key == "v22":
        return symbolic_plan(scenario.rates, v22_model)
...
def run(args: argparse.Namespace) -> int:
    ...
    specs = _load_specs()
    v22_model = require_formal_matrix_model(SIGNAL_EXPERIMENT / "models" / "symbolic_lane_model.json")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--controllers", nargs="+",
        choices=("v22", "webster", "webster-optimized", "hcm", "hcm-optimized", "akcelik", "akcelik-optimized"),
        default=["v22", "webster", "webster-optimized", "hcm", "hcm-optimized", "akcelik", "akcelik-optimized"],
    )
