"""Evaluate each I1--I6 CoSyDelay-V22 model as a frozen transfer model.

The source models are selected from their own Training-only V22 searches.  A
...
SOURCE_RUN = (
    GMini / "methods" / "cosydelay_v22_stable_regeneration" / "experiments"
...
def load_source_model(source_intersection: int) -> tuple[SymbolicModel, dict]:
    ...
    if not path.exists():
        raise FileNotFoundError(f"V22 source result not found: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    expression = data.get("expression") or data.get("selected_expression")
    # ``selected_parameters`` is the complete 12-movement table in the V22
...
def run(args: argparse.Namespace) -> int:
    ...
    manifest = {
        "status": "running",
        "protocol": "true_sumo_tso_frozen_v22_source_sensitivity",
    ...
    for junction in args.junctions:
        for pattern in args.patterns:
            ...
            for seed in args.seeds:
                ...
                row = _run_controller(
                    scenario,
                    f"CoSyDelay-V22-source-I{args.source_intersection}",
                    plan,
                    seed,
                    tripinfo_dir,
                    port=args.port,
