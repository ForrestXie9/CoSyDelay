"""CoSyDelay search with principlewise physical fitness instead of strict joint-pass binary.
...
from methods.cosydelay.run_p10g10_100 import _source as _cosydelay_binary_source


def _source() -> str:
    source = _cosydelay_binary_source()
    replacements = (
        (
        ...
        (
            ...
            "cosydelay_principlewise_physical_mean_p10g10_100_t1",
        ),
        (
            "cosydelay_uniform_restart_broad_regeneration_training_only_p10g10_100_t1",
            "cosydelay_principlewise_physical_mean_training_only_p10g10_100_t1",
        ),
        ("CoSyDelay I{args.intersection}", "CoSyDelay-principlewise I{args.intersection}"),
    )
    for old, new in replacements:
        if old not in source:
            raise RuntimeError(f"CoSyDelay principlewise runner could not locate fragment: {old!r}")
        source = source.replace(old, new)
    return source


def main() -> int:
    namespace = {"__name__": "cosydelay_principlewise_embedded"}
    exec(compile(_source(), "<cosydelay-principlewise>", "exec"), namespace)
    ...
    result.update(
        {
            "formal_version": "CoSyDelay-principlewise",
            "fitness_definition": "training_r2_plus_principlewise_physical_mean",
