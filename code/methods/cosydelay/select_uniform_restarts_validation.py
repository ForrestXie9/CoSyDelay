"""Freeze one CoSyDelay structure per intersection using Validation only.
...
def main() -> int:
    ...
    for intersection in range(1, 7):
        ...
        # Validation selection must refit with the same CoSyDelay role-wise bounds
        ...
        try:
            for restart in range(1, args.restarts + 1):
                ...
                if result.get("formal_version") != "CoSyDelay":
                    raise RuntimeError(f"not a CoSyDelay result: {run_dir}")
    ...
    frozen = {
        "status": "all_cosydelay_structures_frozen_before_test",
        "method": "CoSyDelay-uniform-restarts",
