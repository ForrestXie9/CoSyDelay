"""Prediction-only Test evaluation after six V18 searches are frozen."""
...
METHOD_LABEL = "cosydelay_v18_p10g10"
...
def verify_frozen_search(
    ...
    if protocol.get("status") != launcher.PROTOCOL_STATUS:
        raise RuntimeError("search protocol was not frozen before V18 API calls")
    ...
    bounded = protocol.get("bounded_execution", {})
    if bounded.get("timeout_candidate_counted") is not False:
        raise RuntimeError("frozen V18 protocol does not exclude timed-out candidates")
    prefit_r7 = protocol.get("prefit_r7", {})
    required_prefit_r7 = {
        "enabled": True,
        "uses_training_values": False,
        "uses_targets": False,
        "uses_validation_or_test": False,
        "changes_fitness_definition": False,
    }
    if any(
        prefit_r7.get(key) is not expected
        for key, expected in required_prefit_r7.items()
    ):
        raise RuntimeError("frozen V18 protocol has an invalid R7 pre-fit declaration")
    if batch.get("status") != "complete" or batch.get("formal_training_runs") != 6:
        raise RuntimeError("V18 Training batch is not complete")
    if status.get("status") != "completed" or len(status.get("runs", [])) != 6:
        raise RuntimeError("V18 run status is not complete for all intersections")
    if any(item.get("process_timed_out") for item in status.get("runs", [])):
        raise RuntimeError("a V18 intersection process exceeded its outer limit")
    ...
    if current_source != protocol.get("source_sha256"):
        ...
        raise RuntimeError(
            "frozen V18 source changed before Test evaluation: " + ", ".join(changed)
        )
    results = []
...
def _rename_v16_comparison_fields(output: Path) -> None:
    ...
    for key, value in payload.items():
        renamed[key.replace("v16_", "v18_")] = value
