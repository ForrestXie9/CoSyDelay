...
def _run_pair(
    ...
    for label, prefit_mode in (("A", "none"), ("B", "conservative")):
        ...
        command = [
            sys.executable,
            "-m",
            "methods.cosydelay.engine.optimizer_parallel4.run_training_only_smoke",
            "--intersections",
            str(intersection),
            "--population",
            str(args.population),
...
def _markdown(aggregate: Dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "| I | Mode | Runs | Val R2 mean閸楊槞d | RMSE mean閸楊槞d | Physical | Candidate physical | Accurate+physical | Gate rejects |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for group in aggregate["groups"]:
        label = "A none" if group["prefit_mode"] == "none" else "B conservative"
        lines.append(
            "| I{intersection} | {label} | {runs} | {r2m:.4f} 閸?{r2s:.4f} | "
            "{rmsem:.4f} 閸?{rmses:.4f} | {passes}/{runs} | {cphys:.1%} | "
    ...
    for pair in aggregate["paired_comparisons"]:
        lines.append(
            "- I{intersection}: validation R2 {r2:+.4f} 閸?{r2sd:.4f}; "
            "RMSE {rmse:+.4f} 閸?{rmsesd:.4f}; physical-pass fraction "
            "{physical:+.2f}.".format(
