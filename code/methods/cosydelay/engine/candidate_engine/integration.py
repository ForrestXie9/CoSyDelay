"""Scoped composition of clean V9, the parsimony prompt, and all-log fitting."""
...
from methods.cosydelay.engine.data_protocol.integration import (
    CleanRuntime,
    install_clean_single_evolution,
)
from methods.cosydelay.engine.data_protocol.policy import CleanSearchPolicy
from methods.cosydelay.engine.optimizer_conditioning.complexity_prompt import (
    install_accuracy_first_parsimony_contract,
)

from .fitter import PersistentAllLogFitter
from .final_prompt import (
    MAX_NOVELTY_EXAMPLES_IN_PROMPT,
    format_parent_training_feedback,
    install_final_prompt_contract,
)
...
def install_v10_candidate(
    *,
    ...
    intersection_id: int,
    policy: CleanSearchPolicy = V10_POLICY,
    ...
    if policy.structural_diversity_mode != "family_unique":
        raise ValueError("V10 requires family_unique structural diversity")
    if policy.optimizer_restarts != 10:
        raise ValueError("V10 requires exactly ten optimizer restarts")

    with install_final_prompt_contract(adaptation_module=adaptation_module):
        with install_clean_single_evolution(
            ...
            # The clean runtime object remains the authoritative parent/parameter
            # state store, but its mixed-coordinate worker pool is unused in V10.
            ...
            with install_accuracy_first_parsimony_contract(
                ...
                with PersistentAllLogFitter(
                    ...
                    def fit_with_clean_parent_state(
                        universal_expr: str, **kwargs
                    ):
                    ...
                    def generate_with_parent_training_feedback(*args, **kwargs):
                        ...
                        if mutation_type != "initial" and parent:
                            ...
                            kwargs["search_feedback"] = "\n\n".join(
                                item
                                for item in (parent_feedback, existing)
                                if item
                            )
                    ...
                    population_module.safe_generate_universal_lane_expression = (
                        generate_with_parent_training_feedback
                    )
                    try:
                        yield V10Runtime(
                            clean=clean_runtime,
