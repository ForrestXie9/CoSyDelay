"""Training-only runtime smoke test for the V22 coefficient fitter."""
...
def main() -> int:
    ...
    if diagnostics.get("formal_method_adapter") != "cosydelay_v22_stable_regeneration.fitter":
        raise RuntimeError(f"V22 fitter provenance missing: {diagnostics.get('formal_method_adapter')}")
    payload = {
        "status": "complete_training_only_v22_fitter_smoke",
