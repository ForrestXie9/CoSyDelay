"""V22 accelerated fitter with a pre-screened nonlinear range profile.
...
class StableRangeAcceleratedFitter(PersistentAcceleratedEquivalentFitter):
    ...
    def fit(self, *, universal_expr: str, warm_parameters: Optional[Mapping] = None, **kwargs):
        bounds = coefficient_bounds_for_expression(
            str(universal_expr), V22_RANGE_PROFILE, reject_nonlinear_role_conflicts=False
        )
        ...
        if isinstance(diagnostics, dict):
            diagnostics.update(
                {
                    "formal_method_adapter": "cosydelay_v22_stable_regeneration.fitter",
                    "coefficient_range_profile": V22_RANGE_PROFILE_ID,
                    "coefficient_bounds_by_name": {
                        name: [float(lower), float(upper)]
                        for name, (lower, upper) in bounds.items()
                    },
                    "coefficient_bound_selection_data": "predeclared_after_training_only_paired_screen",
                    "coefficient_range_selection_manifest": dict(V22_RANGE_SELECTION_MANIFEST),
                }
            )
        return result
...
def accelerated_fit_supervisor_main(connection, static_kwargs: dict, settings: dict) -> None:
    """Spawn-safe fitting supervisor mirroring V19 with the V22 profile."""
    fitter = None
    try:
        ...
        while True:
            ...
            try:
                ...
                # dictionary unchanged; recording it here makes the V22
