"""Run one frozen P4/G2 search task using a Train file only."""
...
def main() -> int:
    ...
    targets = {
        approach: train[f"Delay_{approach}"] for approach in approaches
    }
    evolution_seed = (
        args.base_seed
        + args.intersection * 10_000
        + P4G2_ADAPTIVE_SEARCH.generations * 100
        + P4G2_ADAPTIVE_SEARCH.population * 10
        + args.run_id
        - 1
    )
    ...
    try:
        with install_retained_evolution() as runtime:
            result = evolve_universal_lane_expression(
                ...
                pop_size=P4G2_ADAPTIVE_SEARCH.population,
    ...
    if any(item.get("event") == "final_validation_evaluation" for item in history):
        raise RuntimeError("frozen search unexpectedly evaluated validation")
    ...
    output = {
        "method_id": "cosydelay_lbfgsb_r10_parallel4_p4g2_training_selection_v2",
        "method_frozen": True,
        "intersection_id": args.intersection,
        "run_id": args.run_id,
        ...
        "population": P4G2_ADAPTIVE_SEARCH.population,
        "generations": P4G2_ADAPTIVE_SEARCH.generations,
        "optimizer_restarts": OFFICIAL_RESTARTS,
