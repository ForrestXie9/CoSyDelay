"""Run one final V13 P10/G10 Training-only search from scratch."""
...
def main() -> int:
    ...
    if observed != V13_POLICY.llm_sampling:
        raise RuntimeError(
            f"LLM configuration changed: {observed} != {V13_POLICY.llm_sampling}"
        )
    ...
    try:
        ...
        engine.install_v10_candidate = install_v13_candidate
        engine.V10_POLICY = V13_POLICY
        engine._prompt_audit = _v13_prompt_audit
        sys.argv = [
            str(engine.__file__),
            "--intersection",
    ...
    if observed_budget != required:
        raise RuntimeError(f"P10/G10 budget mismatch: {observed_budget} != {required}")
    if result.get("selected_enhanced_physics", {}).get("joint_pass") is not True:
        raise RuntimeError("V13 winner failed the hard physical gate")
    ...
    result.update(
        {
            "status": "completed_v13_p10g10_training_only_search",
            "not_a_formal_result": False,
            "formal_protocol_complete": True,
            "method_id": V13_POLICY.method_id,
            "method_status": "v13_final_p10g10_i1_i6",
            "policy": V13_POLICY.to_dict(),
    ...
    print(
        f"I{args.intersection} V13 P10/G10: "
